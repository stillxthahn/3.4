...
bool Entry::add(FILE* f, VolumeInfo* v, vector<char>& fatTable) {
	int position = -1;
	// Check for empty Cluster first
	...
	// If can not find Empty cluster, write on deleted file's cluster
	if (position == -1)
		for (int i = 0; i < fatTable.size(); i++) {
			if (fatTable[i] == '2') {
				position = i;
	...
	// If every cluster has file 
	if (position == -1)
		return false;
	this->_FatPosition = position;
	fseek(f, v->volumeInfoSize() + fatTable.size() + 2048 * position, SEEK_SET);
